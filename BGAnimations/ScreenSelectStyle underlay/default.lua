...
local choices = {
	...
	},
	{
		...
		x=_screen.cx,
		pads = {
			{color=GetHexColor(SL.Global.ActiveColorIndex-1, true), offset=-51},
			{color=GetHexColor(SL.Global.ActiveColorIndex+2, true), offset=51}
		}
	},
	{
		name="double",
...
if current_game=="dance" and ThemePrefs.Get("AllowDanceSolo") then
	choices[1].x = _screen.cx - 245
	choices[2].x = _screen.cx - 90
	choices[3].x = _screen.cx + 90
	choices[4] = { name="solo", pads={ {color=GetHexColor(SL.Global.ActiveColorIndex, true), offset=0}}, x=_screen.cx + 245 }
elseif current_game=="smx" then
	choices[1].x = _screen.cx - 245
	choices[2].x = _screen.cx - 96
	choices[3] = {
		name="double6", 
		pads = { 
			{color=GetHexColor(SL.Global.ActiveColorIndex, true), offset=-51}, 
			{color=GetHexColor(SL.Global.ActiveColorIndex, true), offset=51}}, 
		x=_screen.cx + 75 }
...
elseif current_game=="kb7" or current_game=="para" then
	choices[1].x = _screen.cx-140
	choices[2].x = _screen.cx+140
